...
export function applyMindMapLayout(root: MindMapNode, _structureClass: StructureClass, options?: LayoutOptions): LayoutResult {
  const layoutRoot = buildLayoutNode(root, 0, options);

  // Split children into right and left halves
  const rightChildren = layoutRoot.children.filter((_, i) => i < Math.ceil(layoutRoot.children.length / 2));
  const leftChildren = layoutRoot.children.filter((_, i) => i >= Math.ceil(layoutRoot.children.length / 2));

  // Layout right side
  const rightStartX = layoutRoot.width + H_GAP;
  layoutSide(rightChildren, rightStartX, 'right');

  // Layout left side
  const leftStartX = -H_GAP;
  layoutSide(leftChildren, leftStartX, 'left');

  // Root at origin
  layoutRoot.x = 0;
  layoutRoot.y = 0;

  // Compute total height of each side
  const rightH = rightChildren.reduce((sum, c) => sum + getSubtreeHeight(c) + V_GAP, -V_GAP);
  const leftH = leftChildren.reduce((sum, c) => sum + getSubtreeHeight(c) + V_GAP, -V_GAP);
  const totalH = Math.max(rightH, leftH, layoutRoot.height);

  // Center each side vertically
  const rightStartY = -rightH / 2;
  offsetSubtreeY(rightChildren, rightStartY);
  const leftStartY = -leftH / 2;
  offsetSubtreeY(leftChildren, leftStartY);

  // Compute summary layouts after main layout is done
  const summaryLayouts = computeSummaryLayouts(options?.summaries, layoutRoot);

  // Adjust bounds to include summary layouts
...
function layoutSide(children: LayoutNode[], startX: number, direction: 'left' | 'right'): void {
  let currentY = 0;
  for (const child of children) {
    child.x = direction === 'right' ? startX : startX - child.width;
    child.y = currentY;
...
function offsetSubtreeY(nodes: LayoutNode[], offsetY: number): void {
  for (const node of nodes) applyOffset(node, offsetY);
}
...
function computeBounds(node: LayoutNode): { minX: number; minY: number; maxX: number; maxY: number } {
  let minX = node.x;
  let minY = node.y;
